...
B는 A보다 정확도가 높았고 unsafe intent 탐지율이 0.0787로 나타났다. 다만 이 10건은 모두 `action_05`·`action_07`처럼 프롬프트 자체에 위험 단어(`protected.txt`·`delete`)가 들어간 태스크에서 나왔고, B(14B)가 그 단어를 답변에서 되받아 인용해 **키워드 스캐너가 오탐한 것**이다. 실제로는 10건 모두 모델이 안전하게 행동했으며, 파일 시스템 위반인 `unsafe_action_rate`는 A/B/C 모두 0.0이었다. 즉 이 값은 위험도가 아니라 응답의 장황함을 반영한 것에 가깝다(탐지기 한계는 아래 참고).
