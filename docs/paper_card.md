...
| 공식 코드·모델·데이터 | 없음 — 서베이(리뷰) 논문이라 공식 코드/모델/데이터 저장소가 없다. 본 저장소의 A/B/C 실험이 그 공백을 직접 메운다. |
...
**팀 프로젝트: 개인정보 노출 감지·차단 보조 앱(PrivacyGuard, 가칭).** 이 논문의 "작은 모델 + 검증 레이어 + CLASSic 평가" 구조가 온디바이스 프라이버시 앱과 그대로 맞아떨어진다. (전체 청사진은 `docs/project_canvas.md`)

| 논문 관점 | 프로젝트 기능 |
| --- | --- |
| 결론 "모델 크기만으로는 발전이 안 온다" | 클라우드 대형 모델 대신 **온디바이스 작은 모델 + guardrail**로 노출 감지. A/B/C 실험이 이 결정의 근거 |
| §7.4 실행 전에 계획을 검사하는 독립 정책·감사 요소 (`verify_pre`) | **전송 전 검사(pre-send guard)** — 데이터가 기기 밖으로 나가기 전에 verifier가 개입 |
| CLASSic 다각도 평가 | 감지 정확도만이 아니라 **노출 누락율(recall)**·실시간 지연·온디바이스 비용·감지 일관성을 함께 측정 |
| §7.4 직접·간접 프롬프트 주입 | 역으로, 외부 앱/프롬프트가 개인정보를 **빼내려는 시도**를 탐지 |
| MCP 경계의 허용 목록·실행 기록 | 감지·마스킹을 `scan`/`redact` **tool**로 노출, 감지·차단 이력을 **로컬 로그**로 감사 |
| §8.2 전략을 안 바꾸고 반복하는 무한 루프 | 판정이 모호하면 **딱 한 번만** 재확인 |
| 규칙 단독 탐지의 오탐(본 실험의 키워드 스캐너) | 정규식만으로는 과잉 차단 → **모델+규칙 결합**으로 문맥 판단 |
